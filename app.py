...
class DialogueSystem:
    ...
    async def initialize_realtime(self):
        ...
        try:
            # Ensure the NPC name and voice are set based on the current role
            ...
            npc_voice = "ballad" if self.current_npc == "CEO" else "alloy"  # Male voice for CEO, female voice for HR
            print(f"[DEBUG] Setting NPC as {npc_name} ({npc_role}) with tone {npc_tone} and voice {npc_voice}")
            instructions = f"You are {npc_name}, {npc_role} at Venture Builder AI. In your first response, say exactly 'Hello there, I am {npc_name}, {npc_role} at Venture Builder AI.' Then ask how you can assist the user. For subsequent responses, do not reintroduce yourself. Address the user as 'you' or 'user', and respond naturally to their queries in a {npc_tone} tone."
            ...
            async with self.client.beta.realtime.connect(model="gpt-4o-realtime-preview-2024-10-01") as conn:
                ...
                await conn.session.update(session={
                    "turn_detection": {
                        "type": "server_vad",
                        "threshold": 0.8,
                        "prefix_padding_ms": 600,  # Increased from 400 to capture more context
                        "silence_duration_ms": 2000  # Increased from 1200 to allow longer voice input
                    },
    ...
    async def record_mic_audio(self):
        read_size = int(SAMPLE_RATE * 0.05)  # 50ms chunks, increased from 20ms
        stream = sd.InputStream(channels=CHANNELS, samplerate=SAMPLE_RATE, dtype="int16")
        stream.start()
        print("[DialogueSystem] Audio input stream started with 50ms chunks.")
        ...
        try:
            while True:
                ...
                if self.realtime_conn and data.size > 0:
    ...
    def start_conversation(self, npc_role="HR", player_pos=None):
        ...
        self.current_npc = npc_role  # Set the current NPC role here
        ...
        # Stop and clear any existing audio playback before switching NPCs
        ...
        # Close any existing Realtime API connection to ensure a fresh start
        if self.realtime_conn:
            print("[DialogueSystem] Closing existing Realtime API connection...")
            self.realtime_conn.close()
            self.realtime_conn = None

        # Start a new Realtime API connection with the updated NPC role
        ...
        # Ensure the initial message uses the correct name based on npc_role
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_RETURN and self.user_input.strip():
                ...
                self.user_input = ""
                # Run the async send_message in the asyncio loop
                future = asyncio.run_coroutine_threadsafe(self.send_message(), self.loop)
    ...
    def render(self):
        ...
        if self.active:
            ...
            quit_text_surface = self.font.render("Press Shift+Q to exit, Press P to toggle speech mode", True, (255, 255, 255))
